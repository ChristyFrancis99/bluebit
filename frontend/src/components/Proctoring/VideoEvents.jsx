...
export function useVideoProctoring(enabled = false) {
  ...
  const startSession = useCallback(async (submissionId = null, assignmentId = null) => {
    ...
    try {
      ...
      flushIntervalRef.current = setInterval(flushEventQueue, 5000)
      
      // Update duration every second
      const durationInterval = setInterval(() => {
        if (startTimeRef.current) {
          const duration = Math.floor((Date.now() - startTimeRef.current) / 1000)
          setMetrics(prev => ({ ...prev, totalDurationSeconds: duration }))
        }
      }, 1000)
      
      // Store interval for cleanup
      flushIntervalRef.current._durationInterval = durationInterval
  ...
  const endSession = useCallback(async () => {
    ...
    try {
      ...
      if (flushIntervalRef.current) {
        if (flushIntervalRef.current._durationInterval) {
          clearInterval(flushIntervalRef.current._durationInterval)
        }
